---
title: "Week 2 Course Project Activity Data"
output:
...
```
The median number of steps per day is **`r format(DailyMedianSteps,scientific=FALSE)` ** and the Mean number of steps per day is **`r format(DailyMeanSteps,scientific=FALSE)` **.
...
`r MaxSteps$steps` steps was maximum observed in a 5 minute interval and that occourred on `r MaxSteps$date` interval number `r MaxSteps$interval`
